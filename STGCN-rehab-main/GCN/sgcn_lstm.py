...
import numpy as np
from tensorflow.keras.layers import Dropout, Dense, Input, LSTM, concatenate, ConvLSTM2D, Softmax
...
class Sgcn_Lstm():
    def __init__(self, train_x, train_y, AD, AD2, bias_mat_1, bias_mat_2, n_classes=2, lr=0.0001, epoach=200,
                 batch_size=10, save_name="best model ex4/best_model.hdf5", class_weight=None):
    ...
    def Lstm(self, x):
        ...
        out = Dense(2, activation='softmax', name='bin_class')(rec3)
        return out
    ...
    def create_model(self):
        ...
        self.model.compile(loss=tf.keras.losses.CategoricalCrossentropy(), optimizer=tf.keras.optimizers.Adam(lr=self.lr))
        return self.model
